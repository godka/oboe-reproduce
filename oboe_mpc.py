...
def main():
    ...
    while True:  # serve video forever
        ...
        if ch_detected or discount is None:
            ch_index = ch_idx
            disc_min, disc_median, disc_max = getDynamicconfig_mpc(
                configmap_mpc_oboe.configmap_mpc_oboe_900, mean_bandwidth, std_bandwidth)
        ...
        max_error = 0
        error_pos = -5
        if (len(past_buffer_errors) < 5):
            error_pos = -len(past_buffer_errors)
        max_error = float(max(past_buffer_errors[error_pos:]))

        start_buffer = buffer_size / (1. + max_error)
        #start = time.time()
        for full_combo in CHUNK_COMBO_OPTIONS:
            ...
            if (reward >= max_reward):
                ...
                past_buffer_ests.append(buffer_tmp)
