...
def getDynamicconfig_bola(pv_list, bw, std, step=900):
    bw_step = step
    std_step = step
    ABRAlgo = ''
    ...
    if True:
        ...
        if (bw_cut, std_cut) not in list(pv_list.keys()):
            for i in range(2, 1000, 1):
                ...
                for bw_ in [bw_cut - (i - 1) * bw_step, bw_cut + (i-1) * bw_step]:
                    for std_ in range(std_cut - (i - 1) * std_step, std_cut + (i-1) * std_step + std_step, std_step):
                        if (bw_, std_) in list(pv_list.keys()):
...
def main():
    ...
    while True:  # serve video forever
        ...
        if ch_detected:
            ch_index = ch_idx
            gp_min, gp_median, gp_max = getDynamicconfig_bola(
                configmap_bola_oboe.configmap_bola_oboe_900, mean_bandwidth, std_bandwidth)
            gp = gp_max
            # print(1. / ( 1. + discount))
        # Vp = getBolaVP(gp)
        score = None
